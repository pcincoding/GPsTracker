...
const ReportFilter = ({ children, handleSubmit, handleSchedule, showOnly, ignoreDevice, multiDevice, includeGroups }) => {
    ...
    if (type === 'schedule') {
      ...
      switch (period) {
        ...
        case 'previousMonth':
          ...
          break;
        default:
          const [formattedFrom, fromTime] = from.split(' ');
          const [formattedTo, toTime] = to.split(' ');
          selectedFrom = new Date(`${NepaliFunctions.BS2AD(formattedFrom)} ${fromTime}`);
          selectedTo = new Date(`${NepaliFunctions.BS2AD(formattedTo)} ${toTime}`);
          break;
  ...
  const handleChange = (e) => {
    ...
    dispatch(reportsActions.updateFrom(`${e.bs}  ${time}`));
  };

  const fromNepaliDatepicker = (input) => {
    if (input === 'custom') {
      setTimeout(() => {
        const datepicker = document.getElementById('nepali-datepicker');
  ...
  const handleChanges = (e) => {
    const date = new Date();
    const time = date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    dispatch(reportsActions.updateTo(`${e.bs}  ${time}`));
  };

  const toNepaliDatepicker = (input) => {
    if (input === 'custom') {
      setTimeout(() => {
        const datepicker2 = document.getElementById('nepali-datepicker2');
        ...
        <>
          <div className={classes.filterItem}>
            <FormControl fullWidth>
              ...
              <Select label={t('reportPeriod')} value={period} onChange={(e) => { dispatch(reportsActions.updatePeriod(e.target.value)); fromNepaliDatepicker(e.target.value); toNepaliDatepicker(e.target.value); }}>
